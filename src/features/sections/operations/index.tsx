...
export function Operations() {
  const { data, updateData, isLoading } = useSection<OperationsType>(
    'operations',
    defaultOperations
  );
  ...
  function addCrewMember() {
    updateData((prev) => ({
      ...prev,
      crew: [...prev.crew, { role: '', hourlyRate: 0, count: 1 }],
    }));
  }
  ...
  function updateCapacity(
    field: keyof OperationsType['capacity'],
    value: number
  ) {
    updateData((prev) => ({
      ...prev,
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              {data.crew.map((member, index) => (
                <div
                  key={index}
                  className="grid grid-cols-1 sm:grid-cols-[1fr_120px_80px_40px] gap-3 items-start border-b pb-3 last:border-0 last:pb-0 sm:border-0 sm:pb-0"
                >
                  <div>
                  ...
                  <div>
                    <span className="text-xs font-medium text-muted-foreground sm:hidden">Hourly Rate</span>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateCrewMember(index, 'hourlyRate', Number(e.target.value))
                        }
                        step="0.5"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={member.count}
                      onChange={(e) =>
                        updateCrewMember(index, 'count', Number(e.target.value))
                      }
                      min={1}
                    />
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <Input
                  type="number"
                  value={data.capacity.maxBookingsPerDay}
                  onChange={(e) => updateCapacity('maxBookingsPerDay', Number(e.target.value))}
                  min={0}
                />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground">Per Week</label>
                <Input
                  type="number"
                  value={data.capacity.maxBookingsPerWeek}
                  onChange={(e) => updateCapacity('maxBookingsPerWeek', Number(e.target.value))}
                  min={0}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={data.capacity.maxBookingsPerMonth}
                  onChange={(e) => updateCapacity('maxBookingsPerMonth', Number(e.target.value))}
                  min={0}
                />
              </div>
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Travel Radius</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="relative">
              <Input
                ...
                onChange={(e) =>
                  updateData((prev) => ({ ...prev, travelRadius: Number(e.target.value) }))
                }
                min={0}
              />
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/50">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800 dark:text-amber-200">
              Slime/chemical activities carry documented dermatitis/burn risk. Ensure proper
              protective gloves, ventilation, and adult supervision for all slime lab activities.
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardContent>
            <div className="space-y-3">
              {data.safetyProtocols.map((protocol, index) => (
                <div key={index} className="flex items-center gap-2">
                  <span className="inline-flex shrink-0 items-center rounded-md bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800 dark:bg-amber-900 dark:text-amber-200">
                    #{index + 1}
                  </span>
                  <Input
                    value={protocol}
              ...
              {data.safetyProtocols.length === 0 && (
                <p className="text-sm text-muted-foreground py-2">
                  No safety protocols defined yet.
                </p>
              )}
            </div>
          </CardContent>
