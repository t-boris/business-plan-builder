...
// Generate 12 months of realistic projections (Mar 2026 - Feb 2027)
function generateDefaultMonths(): MonthlyProjection[] {
  ...
  // Bookings ramp: start conservative, ramp to 25/month by Month 4+
  ...
  return monthNames.map((month, i) => {
    ...
    // Costs breakdown per month
    const marketing = 2200;
    const labor = 3 * 20 * 4 * bookings; // crew(3) * rate($20) * hours(4) * bookings
    ...
    return {
      month,
      revenue,
      costs: { marketing, labor, supplies, museum, transport },
      profit,
    };
  });
}
...
export function FinancialProjections() {
  const { data, updateData, isLoading } = useSection<FinancialProjectionsType>(
  ...
  // Chart data: transform months into chart-friendly format
  ...
  function updateMonthName(index: number, value: string) {
    updateData((prev) => {
      const months = [...prev.months];
      months[index] = { ...months[index], month: value };
      return { ...prev, months };
    });
  }
    ...
    updateData((prev) => ({
      ...
      months: [
        ...prev.months,
        {
          month: 'New Month',
          revenue: 0,
          costs: { marketing: 2200, labor: 0, supplies: 0, museum: 0, transport: 0 },
          profit: -2200,
        },
      ],
    }));
                  ...
                  return (
                    <tr key={i} className="border-b last:border-b-0">
                      <td className="py-1 pr-2">
                        <Input
                          value={m.month}
                          onChange={(e) => updateMonthName(i, e.target.value)}
                      ...
                      <td className="py-1 px-1">
                        <Input
                          type="number"
                          value={m.revenue}
                          onChange={(e) => updateMonth(i, 'revenue', Number(e.target.value))}
                      ...
                      <td className="py-1 px-1">
                        <Input
                          type="number"
                          value={m.costs.marketing}
                          onChange={(e) => updateMonth(i, 'marketing', Number(e.target.value))}
                          className="h-8 text-xs text-right w-20"
                        />
                      </td>
                      <td className="py-1 px-1">
                        <Input
                          type="number"
                          value={m.costs.labor}
                          onChange={(e) => updateMonth(i, 'labor', Number(e.target.value))}
                          className="h-8 text-xs text-right w-20"
                        />
                      </td>
                      <td className="py-1 px-1">
                        <Input
                          type="number"
                          value={m.costs.supplies}
                          onChange={(e) => updateMonth(i, 'supplies', Number(e.target.value))}
                          className="h-8 text-xs text-right w-20"
                        />
                      </td>
                      <td className="py-1 px-1">
                        <Input
                          type="number"
                          value={m.costs.museum}
                          onChange={(e) => updateMonth(i, 'museum', Number(e.target.value))}
                          className="h-8 text-xs text-right w-20"
                        />
                      </td>
                      <td className="py-1 px-1">
                        <Input
                          type="number"
                          value={m.costs.transport}
                          onChange={(e) => updateMonth(i, 'transport', Number(e.target.value))}
                          className="h-8 text-xs text-right w-20"
                        />
                      </td>
                      <td className="py-1 px-2 text-right text-xs font-medium text-muted-foreground">
                        {formatCurrency(totalCosts)}
                      </td>
                      <td className={`py-1 px-2 text-right text-xs font-semibold ${profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {formatCurrency(profit)}
                      </td>
                      <td className="py-1 pl-1">
                        <Button variant="ghost" size="icon-xs" onClick={() => removeMonth(i)}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  type="number"
                  className="pl-7"
                  value={data.unitEconomics.avgCheck}
                  onChange={(e) => updateUnitEconomics('avgCheck', Number(e.target.value))}
                />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  type="number"
                  className="pl-7"
                  value={data.unitEconomics.costPerEvent}
                  onChange={(e) => updateUnitEconomics('costPerEvent', Number(e.target.value))}
                />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  value={data.unitEconomics.profitPerEvent}
                  readOnly
                />
              </div>
            </div>
            <div>
              ...
              <Input
                type="number"
                className="bg-muted"
                value={data.unitEconomics.breakEvenEvents}
                readOnly
              />
            </div>
          </div>
