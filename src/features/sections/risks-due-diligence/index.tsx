...
export function RisksDueDiligence() {
  ...
  if (isLoading) {
    return (
      <div className="page-container">
        <PageHeader showScenarioBadge title="Risks & Due Diligence" description="Loading..." />
      </div>
    );
  }
  ...
  const ddChecklist = data.dueDiligenceChecklist ?? [];
  const vStyle = verdict ? verdictStyles[verdict.verdict] : null;

  return (
    <div className="page-container">
      <PageHeader showScenarioBadge title="Risks & Due Diligence" description="Risk assessment, compliance tracking, and investment verdict" />
      ...
      <div className="page-container">
        ...
        {verdict && vStyle && (
          <div className={`rounded-lg border-l-4 ${vStyle.border} ${vStyle.bg} p-4`}>
            ...
            {verdict.conditions.length === 0 && canEdit && (
              <div className="ml-8">
                <Button variant="ghost" size="sm" onClick={addVerdictCondition} className="text-xs h-7"><Plus className="size-3 mr-1" />Add Condition</Button>
              </div>
            )}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {canEdit && <Button variant="outline" size="sm" onClick={addRisk}><Plus className="size-4" />Add Risk</Button>}
          </div>

          {data.risks.length === 0 ? (
            <EmptyState icon={AlertTriangle} title="No risks identified" description="Use AI Generate to create a risk assessment, or add risks manually." action={canEdit ? { label: 'Add Risk', onClick: addRisk } : undefined} />
          ) : (
            <div className="space-y-3">
              {data.risks.map((risk, index) => (
                <div key={index} className="card-elevated rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {canEdit && <Button variant="ghost" size="icon-xs" onClick={() => removeRisk(index)}><Trash2 className="size-3" /></Button>}
                  </div>
                  ...
                  <div className="form-grid">
                    <div>
                      ...
                      <Select value={risk.severity} onValueChange={(v) => updateRisk(index, 'severity', v)} disabled={!canEdit}>
                    ...
                    <div>
                      ...
                      <Select value={risk.category} onValueChange={(v) => updateRisk(index, 'category', v)} disabled={!canEdit}>
                        <SelectTrigger className="w-full h-8"><SelectValue /></SelectTrigger>
                        <SelectContent>
                  ...
                  {/* Title */}
                  <div>
                    <Input value={risk.title} onChange={(e) => updateRisk(index, 'title', e.target.value)} placeholder="Risk title" className="text-sm font-medium" readOnly={!canEdit} />
                  </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {canEdit && <Button variant="outline" size="sm" onClick={addDDItem}><Plus className="size-4" />Add Item</Button>}
          </div>

          {ddChecklist.length === 0 ? (
            <EmptyState icon={ClipboardCheck} title="No due diligence items" description="Add items to track your due diligence checklist." action={canEdit ? { label: 'Add Item', onClick: addDDItem } : undefined} />
          ) : (
            <div className="space-y-3">
              {ddChecklist.map((item, index) => (
                <div key={index} className="card-elevated rounded-lg p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {canEdit && <Button variant="ghost" size="icon-xs" onClick={() => removeDDItem(index)}><Trash2 className="size-3" /></Button>}
                  </div>
                  <div className="form-grid">
                    <div>
                      <label className="text-xs font-medium text-muted-foreground">Priority</label>
                      <Select value={item.priority} onValueChange={(v) => updateDDItem(index, 'priority', v)} disabled={!canEdit}>
                        <SelectTrigger className="w-full h-8"><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div>
                      ...
                      <Select value={item.status} onValueChange={(v) => updateDDItem(index, 'status', v)} disabled={!canEdit}>
                        <SelectTrigger className="w-full h-8"><SelectValue /></SelectTrigger>
                        <SelectContent>
                  ...
                  <div><Input value={item.item} onChange={(e) => updateDDItem(index, 'item', e.target.value)} placeholder="Due diligence item..." className="text-sm font-medium" readOnly={!canEdit} /></div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                      Detail
                      {canEdit && (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {canEdit && <Button variant="outline" size="sm" onClick={addComplianceItem}><Plus className="size-4" />Add Item</Button>}
          </div>

          {data.complianceChecklist.length === 0 ? (
            <EmptyState icon={CheckCircle2} title="No compliance items" description="Track regulatory and compliance requirements." action={canEdit ? { label: 'Add Item', onClick: addComplianceItem } : undefined} />
          ) : (
              ...
              {data.complianceChecklist.map((item, index) => {
                ...
                return (
                  <div key={index} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <Input value={item.item} onChange={(e) => updateComplianceItem(index, 'item', e.target.value)} placeholder="Compliance item..." className="flex-1 border-0 bg-transparent shadow-none focus-visible:ring-1 px-1 text-sm" readOnly={!canEdit} />
                    <Select value={item.status} onValueChange={(v) => updateComplianceItem(index, 'status', v)} disabled={!canEdit}>
                    ...
                    {canEdit && <Button variant="ghost" size="icon-xs" onClick={() => removeComplianceItem(index)}><Trash2 className="size-3" /></Button>}
                  </div>
                );
              })}
            </div>
