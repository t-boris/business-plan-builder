...
import type { RisksDueDiligence as RisksDueDiligenceType, Risk, RiskSeverity, RiskCategory, ComplianceItem, ComplianceStatus } from '@/types';
...
const severityStyles: Record<RiskSeverity, string> = { high: 'bg-red-100 text-red-800', medium: 'bg-amber-100 text-amber-800', low: 'bg-green-100 text-green-800' };
const categoryStyles: Record<RiskCategory, string> = { regulatory: 'bg-purple-100 text-purple-800', legal: 'bg-blue-100 text-blue-800', operational: 'bg-gray-100 text-gray-800', financial: 'bg-yellow-100 text-yellow-800' };
const complianceStatusStyles: Record<ComplianceStatus, string> = { 'not-started': 'bg-gray-100 text-gray-800', pending: 'bg-amber-100 text-amber-800', complete: 'bg-green-100 text-green-800' };
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Select value={risk.category} onValueChange={(v) => updateRisk(index, 'category', v)} disabled={isPreview}>
                      <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                      <SelectContent><SelectItem value="regulatory">Regulatory</SelectItem><SelectItem value="legal">Legal</SelectItem><SelectItem value="operational">Operational</SelectItem><SelectItem value="financial">Financial</SelectItem></SelectContent>
                    </Select>
                  </div>
                </div>
          ...
          {displayData.risks.length === 0 && <p className="text-sm text-muted-foreground py-4">No risks added. Click "Add Risk" to get started.</p>}
        </div>
      </div>
        ...
        <Card>
          <CardContent>
            <div className="space-y-3">
              ...
              {displayData.complianceChecklist.length === 0 && <p className="text-sm text-muted-foreground py-2">No compliance items. Click "Add Item" to get started.</p>}
            </div>
          </CardContent>
        </Card>
      </div>
