...
const INDUSTRY_OVERLAYS: Partial<Record<BusinessType, Partial<Record<SectionSlug, string>>>> = {
  saas: {
    ...
    operations: 'Workforce should include engineering, product, and customer success roles. Capacity units are \'active users\' or \'API requests\'. Variable costs: cloud infrastructure per user, API costs per request. Fixed costs: SaaS tools, office, insurance. Operational metrics: uptime %, deployment frequency, MTTR, customer churn rate.',
  ...
  restaurant: {
    ...
    operations: 'Workforce should include kitchen and front-of-house roles with shift-based rates. Capacity units are \'covers\' or \'meals\'. Variable costs: food cost per cover, disposables per cover. Fixed costs: rent, utilities, POS, insurance. Operational metrics: food cost %, table turnover, average ticket, waste rate.',
  ...
  event: {
    ...
    operations: 'Workforce should include event crew with per-event rates. Capacity units are \'events\' or \'bookings\'. Variable costs: supplies per event, venue/rental per event, transportation per event. Fixed costs: storage, insurance, marketing, software. Operational metrics: booking fill rate, NPS, repeat booking rate.',
